...
logger = logging.getLogger("obm_germany_etl")
...
class ETLConfig:
    """
    Step 1 ETL:
    OpenBuildingMap S3 -> clean Germany building-level Parquet dataset.
    ...
    # Germany coarse bbox with small margin.
    # Used only as cheap prefilter. Precise filtering is done by BKG boundary.
    lon_min: float = 5.5
    lon_max: float = 15.5
    lat_min: float = 47.0
    lat_max: float = 55.3
    ...
    # DuckDB settings
    duckdb_file: str = "./etl_output/work_obm_germany.duckdb"
    threads: int = 8
...
class OpenBuildingMapGermanyETL:
    ...
    def run(self) -> Path:
        ...
        logger.info("Starting OpenBuildingMap Germany ETL")
        ...
        self._create_germany_boundary_table(
            boundary_gpkg,
            boundary_layer,
            boundary_epsg,
            boundary_geom_col,
    ...
    def _detect_gpkg_boundary_layer(gpkg_path: Path) -> Tuple[str, int, str]:
        """
        Detects the Germany country-level layer in the BKG GeoPackage.

        BKG VG250 usually contains layer names such as vg250_sta, vg250_lan, etc.
        We prefer vg250_sta because that is the national boundary.
        """
    ...
    def _extract_clean_parquet(self, geom_expr: str):
        logger.info("Extracting and enriching Germany buildings to Parquet")
        ...
        COPY (
            ...
            germany_buildings AS (
                SELECT c.*
                FROM candidates c, germany_boundary g
                WHERE
                    c.geom IS NOT NULL
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract Germany buildings from OpenBuildingMap into clean Parquet."
    )
...
def main():
    ...
    cfg = ETLConfig(
        ...
        duckdb_file=str(output_dir / "work_obm_germany.duckdb"),
